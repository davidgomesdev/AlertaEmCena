...
lazy_static! {
    ...
    static ref channel_id: ChannelId =
...
async fn should_read_events() {
    build_api()
        .await
        .get_event_urls_sent(*channel_id)
        .await;
}
...
async fn should_read_event_sent() {
    ...
    let link = format!("https://example.com/events/barca_infern?test_id={}", test_id);
    ...
    api
        .send_event(*channel_id, unique_event)
        .await;

    let is_event_sent = api
        .get_event_urls_sent(*channel_id)
        .await
        .contains(&link);
